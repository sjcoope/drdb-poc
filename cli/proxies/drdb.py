...
def get_database_id(drive_id, db_name):
    resp = table.query(IndexName="gsi_1", KeyConditionExpression=Key("sk").eq("instance#"+drive_id) & Key("data").begins_with("database#"+db_name))
    results = resp["Items"]
...
def get_instance(drive_id):
    resp = table.query(KeyConditionExpression=Key("pk").eq("instance#"+drive_id))
    return resp["Items"]
...
def create_instance(drive_id, resource_id):
    resp = ddb_client.put_item(
        ...
        Item={
            "pk": {"S": "instance#" + drive_id },
            "sk": {"S": "instance#" + resource_id },
            "data": {"S": "resource#" + datetime.today().strftime(date_format) },
            "drive_id": {"S": drive_id},
        }
    )
...
def get_database(drive_id, name):
    resp = table.query(IndexName="gsi_1", KeyConditionExpression=Key("sk").eq("instance#"+drive_id) & Key("data").eq("database#"+name))
    return resp["Items"]

def get_databases(drive_id):
    resp = table.query(IndexName='gsi_1',KeyConditionExpression=Key('sk').eq("instance#"+drive_id) & Key("data").begins_with("database"))
    return resp["Items"]
...
def get_table(drive_id, db_name, table_name):
    database_id = get_database_id(drive_id, db_name)
